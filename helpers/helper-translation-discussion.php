	...
	<div class="comment-content" dir="auto">
		<?php if ( $is_linking_comment ) : ?>
			<?php if ( $reject_reason ) : ?>
				The translation <?php gth_print_translation( $comment_translation_id, $args ); ?> was rejected with <a href="<?php echo esc_url( $comment->comment_content ); ?>"><?php esc_html_e( 'a reason that is being discussed here' ); ?></a>.
			<?php else : ?>
				<a href="<?php echo esc_url( $comment->comment_content ); ?>"><?php esc_html_e( 'Please continue the discussion here' ); ?></a>
			<?php endif; ?>
		<?php else : ?>
			<?php comment_text(); ?>
			<?php if ( $reject_reason ) : ?>
