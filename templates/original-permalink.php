...
<div class="translation-helpers">
	<nav>
		<ul class="helpers-tabs">
			...
			foreach ( $sections as $section ) {
				printf(
					'<li class="%s" data-tab="%s">%s<span class="count">%s</span></li>',
					esc_attr( $is_first_class ),
					esc_attr( $section['id'] ),
					esc_html( $section['title'] ),
					esc_html( $section['count'] ? ( '(' . $section['count'] . ')' ) : '' )
				);
				$is_first_class = '';
			}
			?>
	...
	foreach ( $sections as $section ) {
		...
		echo $section['content']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Content is generated by trusted plugin code.
		if ( $section['has_async_content'] ) {
			echo '</div>';
		}
		echo '</div>';
